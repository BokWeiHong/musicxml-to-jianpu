...
class NoteheadMarkup:
  ...
  def initOneScore(self):
      ...
      self.octavesPosition = "after" # jianpu-patched default # or "before" (only setting in v1.847 and below) or "after", affects chords and grace notes when an octave mark is between two figures: is it before or after the note it affects.  Starting at None = no default, must specify if anything's ambiguous
  ...
  def __call__(self,figures,nBeams,dots,octave,accidental,tremolo,word,line):
    ...
    if isChord:
        chord_ret,octave,placeholder_chord = chordNotes_markup(re.sub(r'[\\qsdhp.]','',word),word,line,self.graceType) # word w/out durations
    ...
    if invisTieLast: # (so figures == "-")
        ...
        if self.current_chord: # a chord is currently in progress, and we're extending it with a tie
            isChord = True
            chord_ret,octave,placeholder_chord = chordNotes_markup(re.sub(r'[\\qsdhp.]','',self.current_chord),word,line)
...
def parseNote(word,origWord,line):
    ...
    nBeams = ''.join(re.findall(r'[cqsdhp\\]',word))
    if re.match(r"[\\]+$",nBeams): nBeams=len(nBeams) # requested by a user who found British note-length names hard to remember; won't work if the \ is placed at the start, as that'll be a Lilypond command
        ...
        try: nBeams = list("cqsdhp").index(nBeams)
...
def xml2jianpu(x):
    ...
    def xmlDuration(whole): # fraction of a whole note -> Lilypond duration string, used for chord mode
        ...
        return best[1]
    types={"128th":"p","64th":"h","32nd":"d","16th":"s","eighth":"q","quarter":"","half":" -","whole":" - - -"}
    typesDot={"128th":"p.","64th":"h.","32nd":"d.","16th":"s.","eighth":"q.","quarter":".","half":" - -","whole":" - - - - -"}
    typesMM={"128th":"128","64th":"64","32nd":"32","16th":"16","eighth":"8","quarter":"4","half":"2","whole":"1"}
    quavers={"128th":F(1,16),"64th":F(1,8),"32nd":F(1,4),"16th":F(1,2),"eighth":F(1),"quarter":F(2),"half":F(4),"whole":F(8)}
    durByQuavers = {F(1,16):"p",F(1,8):"h",F(1,4):"d",F(1,2):"s",F(1):"q",F(2):"",F(4):" -",F(8):" - - -",F(3,8):"h.",F(3,4):"d.",F(3,2):"q.",F(3):".",F(6):" - -",F(12):" - - - - -"}
...
def graceNotes_markup(notes,word,line,isAfter,harmonic=False):
    ...
    while i < len(notes):
        ...
        if chord:
            ...
            for n in chord:
                if n in 'qsdhp': beams='*qsdh'.index(n)
    ...
    while i < len(notes):
        ...
        if chord:
            ...
            for n in chord: # update durations:
                if n in 'qsdhp':
                    duration = {'q':8, 's':16, 'd':32, 'h':64, 'p':128}[n]
        ...
        elif n in 'qsdhp': duration = {'q':8, 's':16, 'd':32, 'h':64, 'p':128}[n]
        else:
...
note_regex = (
    ...
    r"(?:[.,'cqsdhpp#b]" + # non-\ note attribute
    r"[.,'cqsdhpp\\#b]*)?" + # and possibly other note attributes that can include \ (just not as the first character).
    r"[0-9x-]" + # At least one figure (or rest or continuation)
    r"[0-9x.,'cqsdhpp\\#b-]*") # and other figures or attrs after
...
def getLY(score,headers=None,have_final_barline=True):
   ...
   for line in score.split("\n"):
    ...
    if line.startswith("LP:") or line.startswith("LPH:"):
        ...
        # have around anyway:
        line=re.sub(r"(?<=\s)(g\[[#b',1-9qsdhp]+\]\s*)+g\[([#b',1-9qsdhp]+)\](?=\s)",lambda m:re.sub(r"\]\s*g\[","",m.group()),line)
        ...
        for word in line.split():
            ...
            if word.startswith('%'): break # a comment
              ...
              out.append("}")
                ...
                if re.match(r"\\[.,'cqsdhpp\\#b]*[0-9x-][0-9x.,'cqsdhpp\\#b-]*$",word): sys.stderr.write("Warning: '"+word+"' is being interpreted as a Lilypond command.\nIf you meant it as a note, move the \\ away from the start.\n")
            ...
            elif re.match(r"g\[[#b',1-9qsdhp&]+\]$",word):
            ...
            elif re.match(r"\[[#b',1-9qsdhp&]+\]g$",word):
